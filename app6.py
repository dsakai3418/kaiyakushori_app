...
if 'holiday_input_key' not in st.session_state:
...
with col_h_start:
    # keyをカウンターと連動させることで、ボタン押下時にフォームをクリアしたように見せる
...
with col_h_add:
    ...
    if st.button("入力期間で確定", key="confirm_holiday_btn"):
        if new_holiday_start and new_holiday_end and new_holiday_start <= new_holiday_end:
            ...
            st.experimental_rerun() # UIを更新するために再実行
        else:
...
def calculate_min_cancel_date_contract_logic(start_date: datetime.date, holiday_periods: list):
    # 契約開始日から最初の6ヶ月更新日
    # pd.to_datetimeは、datetime.dateを受け取っても動作します
    ...
    for h_start, h_end in holiday_periods:
        # 休業期間と契約期間の重複を考慮した日数計算が必要
        # 例: 契約期間内に完全に含まれる休業期間
        overlap_start = max(pd.to_datetime(start_date).date(), h_start)
        overlap_end = min(pd.to_datetime(current_contract_end).date(), h_end)
        if overlap_start <= overlap_end:
            total_holiday_days_offset += (overlap_end - overlap_start).days + 1
...
def calculate_min_cancel_date_declared_logic(cancel_year: int, cancel_month: int, holiday_periods: list):
    try:
        # 解約希望月の月末日を計算
        # pd.to_datetimeとpd.DateOffsetで柔軟に処理
        declared_cancel_month_end = (pd.to_datetime(f"{cancel_year}-{cancel_month}-01") + pd.DateOffset(months=1) - pd.DateOffset(days=1)).date()
        
        # 休業期間による申告日の延伸（簡易的な実装）
        total_holiday_days_offset = 0
        for h_start, h_end in holiday_periods:
            # 申告日以降に発生する休業期間の影響を考慮
            overlap_start = max(declared_cancel_month_end, h_start)
            overlap_end = h_end # ここはどこまで延長するかによる
            if overlap_start <= overlap_end:
                 total_holiday_days_offset += (overlap_end - overlap_start).days + 1 # 終了日までの日数

        final_declared_cancel_date = declared_cancel_month_end + timedelta(days=total_holiday_days_offset)
