...
apply_cancellation_rule = st.checkbox(
    ...
    help="チェックを外すと、解約希望月をそのまま解約月として計算します（ただし、基準日より過去の場合は基準日）。"
)
...
# 次の更新日を計算するヘルパー関数
# current_reference_date: 計算の基準となる日付 (today_dt または next_renewal_dt など)
def find_next_renewal_date(contract_start_dt: pd.Timestamp, current_reference_date: pd.Timestamp, holiday_periods: list):
    ...
    while True:
        ...
        # 実際には、current_reference_date を超える最初の更新日を探す
        # ただし、契約開始日が current_reference_date より後の場合は、最初の更新日をそのまま返す
        if actual_renewal_dt > current_reference_date or actual_renewal_dt == contract_start_dt + pd.DateOffset(months=6) + pd.Timedelta(days=get_holiday_days_in_period(contract_start_dt, contract_start_dt + pd.DateOffset(months=6) - pd.Timedelta(days=1), holiday_periods)):
            return actual_renewal_dt
        
        cycle_start_dt = actual_renewal_dt

# 最短解約日（契約期間）の計算ロジック
# `today_dt` を `simulation_today_date` に置き換え
def calculate_min_cancel_date_contract_logic(start_date: datetime.date, holiday_periods: list, simulation_today_date: datetime.date):
    contract_start_dt = pd.to_datetime(start_date)
    simulation_today_dt = pd.to_datetime(simulation_today_date)

    # 現在の契約サイクルを超えて、次に到来する更新日を探す (基準日は simulation_today_dt)
    next_renewal_dt = find_next_renewal_date(contract_start_dt, simulation_today_dt, holiday_periods)
    ...
    # 最短解約日（契約期間）は、基準日より過去になることはない
    return max(min_cancel_dt.date(), simulation_today_dt.date())

# 最短解約日（申告日）の計算ロジック
# `today_dt` を `simulation_today_date` に置き換え
def calculate_min_cancel_date_declared_logic(contract_start_date: datetime.date, cancel_year: int, cancel_month: int, holiday_periods: list, apply_cancellation_rule: bool, simulation_today_date: datetime.date):
    try:
        ...
        if not apply_cancellation_rule:
            ...
            # ただし、基準日よりも過去の場合は基準日とする
            return max(requested_cancel_dt_eom.date(), simulation_today_dt.date())

        # ルールを適用する場合（これまでのロジック）
        # 現在の契約サイクルで次に到来する更新日を見つける (基準日は simulation_today_dt)
        next_contract_renewal_dt = find_next_renewal_date(contract_start_dt, simulation_today_dt, holiday_periods)
...
if st.button("全ての計算を実行", key="execute_calculation_btn", type="primary"):
    with st.spinner('計算中...しばらくお待ちください。'):
        ...
        # 契約残存期間の計算を実行
        # シミュレーション基準日を渡す
        calculated_min_cancel_date_contract = calculate_min_cancel_date_contract_logic(contract_start_date, st.session_state.holiday_periods, simulation_today_date)
        calculated_min_cancel_date_declared = calculate_min_cancel_date_declared_logic(contract_start_date, cancel_year, cancel_month, st.session_state.holiday_periods, apply_cancellation_rule, simulation_today_date)
        ...
        if calculated_min_cancel_date_declared:
            # `today_date` を `simulation_today_date` に置き換え
            if calculated_min_cancel_date_declared >= simulation_today_date:
                # pandas Period を使って正確な月数を計算
                start_period = pd.Period(simulation_today_date, 'M')
