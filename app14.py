...
with upload_col2:
    ...
    bakuraku_csv_file = st.file_uploader("バクラクからの請求CSVをここにドラッグ＆ドラッグ、またはファイルを選択", type=["csv"], key="bakuraku_csv")
...
def calculate_min_cancel_date_contract_logic(start_date: datetime.date, holiday_periods: list):
    ...
    current_cycle_start_dt = contract_start_dt
    
    # 現在の契約サイクル開始日を見つける
    while True:
        next_cycle_boundary_dt = current_cycle_start_dt + pd.DateOffset(months=6)
        
        # この期間内の休業日数を取得
        holidays_in_current_cycle = get_holiday_days_in_period(current_cycle_start_dt, next_cycle_boundary_dt - pd.Timedelta(days=1), holiday_periods)
        
        effective_next_cycle_boundary_dt = next_cycle_boundary_dt + pd.Timedelta(days=holidays_in_current_cycle)
        
        if effective_next_cycle_boundary_dt > today_dt:
            # 現在の契約サイクルが確定
            break
        current_cycle_start_dt = effective_next_cycle_boundary_dt

    # 次の更新日 (現在のサイクル終了日 + 1日)
    next_renewal_dt = current_cycle_start_dt + pd.DateOffset(months=6)
    holidays_for_next_renewal = get_holiday_days_in_period(current_cycle_start_dt, next_renewal_dt - pd.Timedelta(days=1), holiday_periods)
    final_renewal_dt = next_renewal_dt + pd.Timedelta(days=holidays_for_next_renewal)
    ...
    # 計算された最短解約日が今日より過去であれば、さらに次の更新日を考慮する
    # （ただし、上記のループで現在以降のサイクルを見つけているはずなので、基本的には不要だが念のため）
    while min_cancel_dt.date() < today_dt.date():
        current_cycle_start_dt = final_renewal_dt # 次のサイクルの開始日は前のサイクルの終了日
        next_renewal_dt = current_cycle_start_dt + pd.DateOffset(months=6)
        holidays_for_next_renewal = get_holiday_days_in_period(current_cycle_start_dt, next_renewal_dt - pd.Timedelta(days=1), holiday_periods)
        final_renewal_dt = next_renewal_dt + pd.Timedelta(days=holidays_for_next_renewal)
...
def calculate_min_cancel_date_declared_logic(contract_start_date: datetime.date, cancel_year: int, cancel_month: int, holiday_periods: list):
    try:
        ...
        requested_cancel_dt = pd.to_datetime(datetime(cancel_year, cancel_month, 1))

        # ユーザーの解約希望月が属する更新サイクルを特定
        current_cycle_start_dt = contract_start_dt
        ...
        while True:
            ...
            # もし effective_next_cycle_boundary_dt がユーザーの解約希望日を過ぎていれば、
            # このサイクルが解約希望日を含むサイクル、あるいはその次のサイクルになる
            if effective_next_cycle_boundary_dt.to_period('M') >= requested_cancel_dt.to_period('M'):
                 # ユーザーの解約希望が更新月よりも前なら、このサイクルを適用
                 # そうでなければ、次のサイクルを適用
                if requested_cancel_dt.to_period('M') <= (effective_next_cycle_boundary_dt - pd.DateOffset(months=1)).to_period('M'):
                    break # このサイクルが適用対象
                else: # 解約希望月が更新月の1ヶ月前ルールに間に合わない場合
                    current_cycle_start_dt = effective_next_cycle_boundary_dt # 次のサイクルへ
                    
            else:
                current_cycle_start_dt = effective_next_cycle_boundary_dt

        # 最終的な更新日（current_cycle_start_dt + 6ヶ月 + 休業期間）
        final_renewal_dt = current_cycle_start_dt + pd.DateOffset(months=6)
        holidays_for_renewal = get_holiday_days_in_period(current_cycle_start_dt, final_renewal_dt - pd.Timedelta(days=1), holiday_periods)
        final_renewal_dt_adjusted = final_renewal_dt + pd.Timedelta(days=holidays_for_renewal)

        # 「更新月の1ヶ月前までの申し出で解約可能」ルールを適用
        min_cancel_declared_dt = final_renewal_dt_adjusted - pd.DateOffset(months=1)
    ...
    with st.spinner('計算中...しばらくお待ちください。'):
        ...
        if calculated_min_cancel_date_declared:
            ...
            if calculated_min_cancel_date_declared >= today_date:
                ...
                start_period = pd.Period(today_date, 'M')
