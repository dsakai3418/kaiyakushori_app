...
# --- 初期化 ---
# アプリの初期ロード時またはセッションリセット時にのみ実行されるようにする
...
with col_h_start:
    ...
    # value=Noneを設定することで、キー変更時に初期値なしになる
...
def calculate_min_cancel_date_contract_logic(start_date: datetime.date, holiday_periods: list):
    ...
    # ここで current_contract_end_dt は今日以降で最も近い6ヶ月更新日
    ...
    for h_start, h_end in holiday_periods:
        ...
        # 休業期間が現在の契約サイクル終了日以降に始まる、または終了日をまたぐ場合
        # このロジックは非常に複雑になるため、ここでは簡易的に「契約サイクル終了日以降の休業期間の日数を加算」
        # より正確な実装では、契約期間全体を見て休業期間による契約期間の「実質的な」延長を計算する
        if h_start_dt > current_contract_end_dt:
            ...
            # 契約期間終了日にまたがる休業期間
            total_holiday_days_offset += (h_end_dt - current_contract_end_dt).days # current_contract_end_dtの翌日から計算
    ...
    try:
        ...
        for h_start, h_end in holiday_periods:
            ...
            # 申告希望日にまたがる休業期間
                ...
                total_holiday_days_offset += (h_end_dt - declared_cancel_month_end_dt).days # 申告希望日の翌日から計算
