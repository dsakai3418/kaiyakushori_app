...
# 次の更新日を計算するヘルパー関数
# current_reference_dt: 計算の基準となる日付 (contract_start_dt_ts など)
def find_next_renewal_date(contract_start_dt_ts: pd.Timestamp, current_reference_dt: pd.Timestamp, holiday_periods: list):
    ...
    # 契約開始日が current_reference_dt より後の場合、最初の更新日を返すべき
    if contract_start_dt_ts > current_reference_dt:
        # 最初の6ヶ月サイクルを計算
        projected_renewal_dt_initial = contract_start_dt_ts + pd.DateOffset(months=6)
        holidays_initial = get_holiday_days_in_period(contract_start_dt_ts, projected_renewal_dt_initial - pd.Timedelta(days=1), holiday_periods)
        return projected_renewal_dt_initial + pd.Timedelta(days=holidays_initial)

    # current_reference_dt 以前のサイクルをスキップし、current_reference_dt を超える最初の更新日を探す
    while True:
...
# 最短解約日（契約期間）の計算ロジック
# 引数を pd.Timestamp に変更し、比較も pd.Timestamp で行う
def calculate_min_cancel_date_contract_logic(start_date_ts: pd.Timestamp, holiday_periods: list):
    # 現在の契約サイクルを超えて、次に到来する更新日を探す (基準日は start_date_ts)
    next_renewal_dt = find_next_renewal_date(start_date_ts, start_date_ts, holiday_periods)
...
def calculate_min_cancel_date_declared_logic(contract_start_date_ts: pd.Timestamp, cancel_year: int, cancel_month: int, holiday_periods: list, apply_cancellation_rule: bool):
    try:
        ...
        # `current_reference_dt_ts` (契約開始日) を起点に更新日を見つける
        # ユーザーの希望月が契約開始日より前の場合、contract_start_date_ts を基準とする
        actual_reference_for_renewal_ts = max(current_reference_dt_ts, requested_cancel_dt_eom)
        
        next_contract_renewal_dt = find_next_renewal_date(contract_start_date_ts, actual_reference_for_renewal_ts, holiday_periods)
...
if st.button("全ての計算を実行", key="execute_calculation_btn", type="primary"):
    with st.spinner('計算中...しばらくお待ちください。'):
        ...
        # 契約残存期間の計算を実行
        # contract_start_date_ts (pandas.Timestamp) を渡す
        calculated_min_cancel_date_contract = calculate_min_cancel_date_contract_logic(contract_start_date_ts, st.session_state.holiday_periods)
        calculated_min_cancel_date_declared = calculate_min_cancel_date_declared_logic(contract_start_date_ts, cancel_year, cancel_month, st.session_state.holiday_periods, apply_cancellation_rule)
