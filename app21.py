...
def calculate_min_contract_end_date(start_date_ts: pd.Timestamp, cancel_year: int, cancel_month: int, holiday_periods: list):
    try:
        # ユーザーの解約希望月の月末日
        ...
        if requested_cancel_dt_eom <= deadline_for_current_renewal:
            ...
            next_next_renewal_dt = find_next_renewal_date(start_date_ts, next_contract_renewal_dt + pd.Timedelta(days=1), holiday_periods) # 次の更新日より1日後を基準
