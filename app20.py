...
# 目的: current_reference_dt (またはそれ以降) を超える最初の更新日を探す
def find_next_renewal_date(contract_start_dt_ts: pd.Timestamp, current_reference_dt: pd.Timestamp, holiday_periods: list):
    ...
    # current_reference_dt を超える最初の更新日を探すループ
    while True:
        ...
        # 重要な変更: 「超える」ではなく「以上」で比較を始める
        # if actual_renewal_dt > current_reference_dt:
        # ユーザーの希望日が更新日と一致する場合もそのサイクルに含める
        if actual_renewal_dt >= current_reference_dt: # 変更点: >= に変更
...
# 最短解約日（契約期間）の計算ロジック
# この関数は、ユーザーが指定した解約希望月を考慮し、その月に最も近い（またはその月を含む）契約サイクルの解約日を計算します。
def calculate_min_cancel_date_contract_logic(start_date_ts: pd.Timestamp, cancel_year: int, cancel_month: int, holiday_periods: list):
    # ユーザーの解約希望月の月末日を計算基準とする
    requested_cancel_dt_eom = pd.to_datetime(datetime(cancel_year, cancel_month, 1)).to_period('M').end_time
    
    # 現在の契約期間の終了日 (最初の6ヶ月サイクル) をまず計算
    # find_next_renewal_date を使って、契約開始日以降の最初の更新日を探す
    first_renewal_dt = find_next_renewal_date(start_date_ts, start_date_ts, holiday_periods)
    first_contract_end_dt = first_renewal_dt - pd.Timedelta(days=1) # 最初の契約期間の最終日

    # もし希望解約月が最初の契約期間の最終月よりも後であれば、次の契約更新サイクルを考慮する
    if requested_cancel_dt_eom > first_contract_end_dt:
        # ユーザーの希望解約月が次期契約期間に該当する場合、
        # `requested_cancel_dt_eom` を基準として、それ以降の更新日を探しに行く
        reference_for_renewal_search = requested_cancel_dt_eom # ここを起点に次の更新サイクルを探す
    else:
        # ユーザーの希望解約月が最初の契約期間内であれば、最初の契約期間の解約日を計算する
        reference_for_renewal_search = start_date_ts # 契約開始日を起点に更新日を探す

    # 基準となる日付を超過する次の更新日を探す
    next_renewal_dt = find_next_renewal_date(start_date_ts, reference_for_renewal_search, holiday_periods)
    ...
    return max(min_cancel_dt, start_date_ts).date() # 最終的には日付オブジェクトに戻す

# 最短解約日（申告日）の計算ロジック
def calculate_min_cancel_date_declared_logic(contract_start_date_ts: pd.Timestamp, cancel_year: int, cancel_month: int, holiday_periods: list, apply_cancellation_rule: bool):
    try:
        # ユーザーの解約希望月の月末日
        requested_cancel_dt_eom = pd.to_datetime(datetime(cancel_year, cancel_month, 1)).to_period('M').end_time
        
        # 計算の基準日 (契約開始日)
        current_reference_dt_ts = contract_start_date_ts
        ...
        # 契約開始日と解約希望月の月末日のうち、より新しい方を参照点とする
        # ここではrequested_cancel_dt_eomがそのまま参照点となることが多い
        actual_reference_for_renewal_search = requested_cancel_dt_eom # 変更なし

        # 参照点を超える（または含む）最初の更新日を探す
        next_contract_renewal_dt = find_next_renewal_date(contract_start_date_ts, actual_reference_for_renewal_search, holiday_periods)
        ...
        if requested_cancel_dt_eom <= deadline_for_current_renewal:
            ...
            next_next_renewal_dt = find_next_renewal_date(contract_start_date_ts, next_contract_renewal_dt, holiday_periods)
...
if st.button("全ての計算を実行", key="execute_calculation_btn", type="primary"):
    with st.spinner('計算中...しばらくお待ちください。'):
        ...
        # 契約残存期間の計算を実行
        # `calculated_min_cancel_date_contract` にも解約希望月を渡す
        calculated_min_cancel_date_contract = calculate_min_cancel_date_contract_logic(
            contract_start_date_ts, cancel_year, cancel_month, st.session_state.holiday_periods
        )
        calculated_min_cancel_date_declared = calculate_min_cancel_date_declared_logic(
        ...
        if calculated_min_cancel_date_declared:
            # `contract_start_date_dt` (datetime.date) を基準として月数を計算
            if calculated_min_cancel_date_declared >= contract_start_date_dt:
