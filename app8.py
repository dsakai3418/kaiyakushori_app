...
if st.button("全ての計算を実行", key="execute_calculation_btn", type="primary"):
    with st.spinner('計算中...しばらくお待ちください。'):
        ...
        billing_entity = ""
        if unpaid_amount == 0:
            billing_entity = "（未入金なし）"
        elif np_df is not None and bakuraku_df is not None:
            billing_entity = "（バクラク）"
        ...
        st.markdown(f"◆ 支払計画：**{payment_plan_amount:,.0f}円** （残存**{remaining_months_rough}ヶ月**）{billing_entity}")
else:
