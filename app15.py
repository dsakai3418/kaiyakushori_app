...
apply_cancellation_rule = st.checkbox(
    ...
    help="チェックを外すと、解約希望月をそのまま解約月として計算します（ただし、今日より過去の場合は今日）。"
)
...
# 次の更新日を計算するヘルパー関数
def find_next_renewal_date(contract_start_dt: pd.Timestamp, current_date: pd.Timestamp, holiday_periods: list):
    # 現在の契約サイクル開始日を見つける
    ...
    while True:
        ...
        if actual_renewal_dt > current_date:
            return actual_renewal_dt
        cycle_start_dt = actual_renewal_dt # 次のサイクルへ
...
def calculate_min_cancel_date_contract_logic(start_date: datetime.date, holiday_periods: list):
    ...
    # 現在の契約サイクルを超えて、次に到来する更新日を探す
    next_renewal_dt = find_next_renewal_date(contract_start_dt, today_dt, holiday_periods)
...
def calculate_min_cancel_date_declared_logic(contract_start_date: datetime.date, cancel_year: int, cancel_month: int, holiday_periods: list, apply_cancellation_rule: bool):
    try:
        ...
        if not apply_cancellation_rule:
            ...
            # ただし、今日よりも過去の場合は今日の日付とする
            return max(requested_cancel_dt_eom.date(), today_dt.date())

        # ルールを適用する場合（これまでのロジック）
        # 現在の契約サイクルで次に到来する更新日を見つける
        next_contract_renewal_dt = find_next_renewal_date(contract_start_dt, today_dt, holiday_periods)
