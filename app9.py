...
if st.button("全ての計算を実行", key="execute_calculation_btn", type="primary"):
    with st.spinner('計算中...しばらくお待ちください。'):
        ...
        # --- 支払計画に発行元を追加するロジック（修正版） ---
        billing_entity_info = ""
        if unpaid_amount == 0:
            billing_entity_info = "（未入金なし）"
        else: # 未入金がある場合
            if np_df is not None and bakuraku_df is not None:
                ...
                billing_entity_info = "（バクラク）"
            else: # CSVファイルがどちらもアップロードされていないが、未入金がある（例えば手入力のみの場合）
                billing_entity_info = "（不明）" # あるいは表示しない、など
        
        st.markdown(f"◆ 支払計画：**{payment_plan_amount:,.0f}円** （残存**{remaining_months_rough}ヶ月**）{billing_entity_info}")
else:
